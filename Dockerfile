...
RUN (mkdir -p /build/flux-mail-rest-api/libs/flux-autoload-api && cd /build/flux-mail-rest-api/libs/flux-autoload-api && wget -O - https://github.com/flux-eco/flux-autoload-api/releases/download/v2022-07-12-1/flux-autoload-api-v2022-07-12-1-build.tar.gz | tar -xz --strip-components=1 && /flux-namespace-changer/bin/change-namespace.php . FluxAutoloadApi FluxMailRestApi\\Libs\\FluxAutoloadApi)

RUN (mkdir -p /build/flux-mail-rest-api/libs/flux-mail-api && cd /build/flux-mail-rest-api/libs/flux-mail-api && wget -O - https://github.com/flux-eco/flux-mail-api/releases/download/v2022-07-12-1/flux-mail-api-v2022-07-12-1-build.tar.gz | tar -xz --strip-components=1 && /flux-namespace-changer/bin/change-namespace.php . FluxMailApi FluxMailRestApi\\Libs\\FluxMailApi)
